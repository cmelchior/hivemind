...
/**
 * What token is the last to be placed, ending the game.
 */
public class LastTokenMetric extends Metric {

    private static final boolean DEBUG = false;

    public static final String FILE_NAME = "closing_token.data";
    public static final String WEIGHTED_FILE_NAME = "weighted_closing_token.data";
    ...
    public void analyzeGame(BoardspaceGameType type, String variant, Game game) {
        ...
        boolean isGameOver = status == GameStatus.RESULT_BLACK_WINS || status == GameStatus.RESULT_WHITE_WINS || status == GameStatus.RESULT_DRAW;
        GameCommand c = game.getMove(game.getOtherPlayer(), game.getOtherPlayer().getTurns());
        if (c != null && isGameOver) {
            String tokenType = (c.isMovedByPillbug()) ? "P" : c.getToken().getId().substring(0, 1);
