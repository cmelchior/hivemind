...
set terminal png size 1000,400 enhanced font "Verdana,10"
set output output;

#Plot
plot for [COL=STARTCOL:ENDCOL] '../results/game_results.data' u COL:xtic(1) w histogram title columnheader(COL), \
     for [COL=STARTCOL:ENDCOL] '../results/game_results.data' u (column(0)-1+BOXWIDTH*(COL-STARTCOL+GAPSIZE/2+1)-0.5):COL:(sprintf("%.1f", column(COL))) notitle w labels center offset 0,0.5